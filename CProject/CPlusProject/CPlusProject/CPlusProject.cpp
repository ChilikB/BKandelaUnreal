...
auto playerCharacter = std::make_unique<PlayerCharacter>();
...
void main()
{
	auto observer = std::make_unique<PlayerCharacterObserver>();
	playerCharacter->addObserver(observer.get());
	//while (menu());
	//while (menu2());
	//while (menu4());
	//while (menu5());
...
int menu6() {
	try {
		...
		std::cout << "1. Show character info" << std::endl;
		std::cout << "2. Show character inventory" << std::endl;
		std::cout << "3. Equip item" << std::endl;
		std::cout << "4. Unequip item" << std::endl;
		std::cout << "5. Move character" << std::endl;
		...
		case 1:
		{
			system("cls");
			Utils::SetConsoleColor(2, 0);
			playerCharacter->showCharacterInfo();
			Utils::ResetConsoleColor();
		}
		break;
		case 2:
		...
		case 3:
		{
			...
			Utils::SetConsoleColor(2, 0);
			int itemId;
			std::cout << "Enter item ID to equip:" << std::endl;
			std::cin >> itemId;
			if (std::cin.fail()) {
				throw std::invalid_argument("Invalid input for item ID. Please enter a valid number.");
			}
			auto isSuccess = playerCharacter->equip(itemId);
			if (isSuccess) {
				std::cout << "Item equipped." << std::endl;
			}
			else 
			{
				std::cout << "Failed to equip item. Item not found in inventory." << std::endl;
			}
			Utils::ResetConsoleColor();
		}
		break;
		case 4:
		{
			...
			Utils::SetConsoleColor(2, 0);
			int itemId;
			std::cout << "Enter item ID to unequip:" << std::endl;
			std::cin >> itemId;
			if (std::cin.fail()) {
				throw std::invalid_argument("Invalid input for item ID. Please enter a valid number.");
			}
			auto isSuccess = playerCharacter->unequip(itemId);
			if(isSuccess)
				std::cout << "Item unequipped." << std::endl;
			else
				std::cout << "Failed to unequip item. Item not found." << std::endl;
			Utils::ResetConsoleColor();
		}
		break;
		case 5:
		{
			...
			Utils::SetConsoleColor(2, 0);
			int x, y;
			std::cout << "Enter movement in X direction:" << std::endl;
			std::cin >> x;
			if (std::cin.fail()) {
				throw std::invalid_argument("Invalid input for X movement. Please enter a valid number.");
			}
			std::cout << "Enter movement in Y direction:" << std::endl;
			std::cin >> y;
			if (std::cin.fail()) {
				throw std::invalid_argument("Invalid input for Y movement. Please enter a valid number.");
			}
